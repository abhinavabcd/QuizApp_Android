...
import com.amcolabs.quizapp.R.styleable;
...
public class TimerView extends View{
    ...
    private Paint[]  barPaints = new Paint[10];
    private int[] barColors = new int[10];
    private int[] nProgress = new int[10];
    private int nTimers = 1;
...
//    private Paint barPaint = new Paint();
	...
	public void resetTimer(int timerInseconds){
    	while(nTimers>1)
    		nProgress[nTimers--] = 0;
        setText(Integer.toString(timerInseconds));
	...
	public void startTimer(int timerSeconds){
		...
		final Runnable r = new Runnable() {
			...
			public void run() {
				while(true) {
					if(!running) return;
					if(temp>500){//just reducing calls
						temp=0;
						elapsedTimeInSeconds = Config.getElapsedTimeInSec(Config.getCurrentNanos()-startTime);
						setText( Integer.toString(timerInSeconds - (int)elapsedTimeInSeconds));
					}
					incrementProgress();
					try {
						Thread.sleep(15);
	...
	public double getTimeElapsed(int timerIndex){
    ...
    private void addNewTimer(){
    	...
    	nProgress[nTimers]= nProgress[nTimers-1];
    ...
    private void setupPaints() {
    	barPaints[0] = new Paint();
        barPaints[0].setColor(barColors[0]);
        barPaints[0].setAntiAlias(true);
        barPaints[0].setStyle(Style.STROKE);
        barPaints[0].setStrokeWidth(barWidth);
    ...
    private void parseAttributes(TypedArray a) {
        ...
        barColors[0] = a.getColor(R.styleable.ProgressWheel_barColor, barColors[0]);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //Draw the bar
        if (isSpinning) {
            canvas.drawArc(circleBounds, nProgress[0] - 90, barLength, false,
                    barPaints[0]);
        } else {
        	for(int i=nTimers-1;i>=0;i--)
        		canvas.drawArc(circleBounds, -90, nProgress[i], false, barPaints[i]);
        }
